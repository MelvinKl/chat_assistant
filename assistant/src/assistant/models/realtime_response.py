...
class RealtimeResponse(BaseModel):
    ...
    conversation_id: Optional[StrictStr] = Field(default=None, description="Which conversation the response is added to, determined by the `conversation` field in the `response.create` event. If `auto`, the response will be added to the default conversation and the value of `conversation_id` will be an id like `conv_1234`. If `none`, the response will not be added to any conversation and the value of `conversation_id` will be `null`. If responses are being triggered by server VAD, the response will be added to the default conversation, thus the `conversation_id` will be an id like `conv_1234`. ")
    voice: Optional[VoiceIdsShared] = None
    modalities: Optional[List[StrictStr]] = Field(default=None, description="The set of modalities the model used to respond. If there are multiple modalities, the model will pick one, for example if `modalities` is `[\"text\", \"audio\"]`, the model could be responding in either text or audio. ")
    ...
    __properties: ClassVar[List[str]] = ["id", "object", "status", "status_details", "output", "metadata", "usage", "conversation_id", "voice", "modalities", "output_audio_format", "temperature", "max_output_tokens"]
    ...
    def from_dict(cls, obj: Dict) -> Self:
        ...
        _obj = cls.model_validate({
            ...
            "max_output_tokens": RealtimeResponseMaxOutputTokens.from_dict(obj.get("max_output_tokens")) if obj.get("max_output_tokens") is not None else None
        })
        return _obj
