...
from assistant.models.chat_completion_message_tool_call import ChatCompletionMessageToolCall
from assistant.models.chat_completion_response_message_annotations_inner import (
    ChatCompletionResponseMessageAnnotationsInner,
)
from assistant.models.chat_completion_response_message_audio import ChatCompletionResponseMessageAudio
from assistant.models.chat_completion_response_message_function_call import ChatCompletionResponseMessageFunctionCall
...
class ChatCompletionMessageListDataInner(BaseModel):
    """
    ...
    annotations: Optional[List[ChatCompletionResponseMessageAnnotationsInner]] = Field(
        default=None,
        description="Annotations for the message, when applicable, as when using the [web search tool](/docs/guides/tools-web-search?api-mode=chat). ",
    )
    ...
    def from_json(cls, json_str: str) -> Self:
        """Create an instance of ChatCompletionMessageListDataInner from a JSON string"""
    ...
    def from_dict(cls, obj: Dict) -> Self:
        """Create an instance of ChatCompletionMessageListDataInner from a dict"""
        if obj is None:
            return None
        ...
        _obj = cls.model_validate(
            {
                ...
                "tool_calls": (
                    [ChatCompletionMessageToolCall.from_dict(_item) for _item in obj.get("tool_calls")]
                    if obj.get("tool_calls") is not None
                    else None
                ),
                "annotations": (
                    [ChatCompletionResponseMessageAnnotationsInner.from_dict(_item) for _item in obj.get("annotations")]
                    if obj.get("annotations") is not None
                    else None
                ),
                "role": obj.get("role"),
                "function_call": (
                    ChatCompletionResponseMessageFunctionCall.from_dict(obj.get("function_call"))
                    if obj.get("function_call") is not None
                    else None
                ),
                "audio": (
                    ChatCompletionResponseMessageAudio.from_dict(obj.get("audio"))
