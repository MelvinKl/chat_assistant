...
class RealtimeServerEventResponseTextDelta(BaseModel):
    ...
    type: StrictStr = Field(description="The event type, must be `response.text.delta`.")
    ...
    delta: StrictStr = Field(description="The text delta.")
    __properties: ClassVar[List[str]] = ["event_id", "type", "response_id", "item_id", "output_index", "content_index", "delta"]

    @field_validator('type')
    def type_validate_enum(cls, value):
        """Validates the enum"""
        if value not in ('response.text.delta',):
            raise ValueError("must be one of enum values ('response.text.delta')")
        return value
