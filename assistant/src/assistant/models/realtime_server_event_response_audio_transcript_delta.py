...
class RealtimeServerEventResponseAudioTranscriptDelta(BaseModel):
    ...
    type: StrictStr = Field(description="The event type, must be `response.audio_transcript.delta`.")
    ...
    delta: StrictStr = Field(description="The transcript delta.")
    __properties: ClassVar[List[str]] = ["event_id", "type", "response_id", "item_id", "output_index", "content_index", "delta"]

    @field_validator('type')
    def type_validate_enum(cls, value):
        """Validates the enum"""
        if value not in ('response.audio_transcript.delta',):
            raise ValueError("must be one of enum values ('response.audio_transcript.delta')")
        return value
