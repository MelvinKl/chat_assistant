...
from assistant.models.realtime_response_status_details_error import RealtimeResponseStatusDetailsError
try:
...
class RealtimeResponseStatusDetails(BaseModel):
    ...
    """ # noqa: E501
    type: Optional[StrictStr] = Field(default=None, description="The type of error that caused the response to fail, corresponding  with the `status` field (`completed`, `cancelled`, `incomplete`,  `failed`). ")
    ...
    def reason_validate_enum(cls, value):
        ...
        if value not in ('turn_detected', 'client_cancelled', 'max_output_tokens', 'content_filter',):
            raise ValueError("must be one of enum values ('turn_detected', 'client_cancelled', 'max_output_tokens', 'content_filter')")
        return value
