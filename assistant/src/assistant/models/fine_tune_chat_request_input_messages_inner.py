...
from assistant.models.fine_tune_chat_completion_request_assistant_message import (
...
FINETUNECHATREQUESTINPUTMESSAGESINNER_ONE_OF_SCHEMAS = [
    ...
    def actual_instance_must_validate_oneof(cls, v):
        ...
        if not isinstance(v, FineTuneChatCompletionRequestAssistantMessage):
            error_messages.append(
                f"Error! Input type `{type(v)}` is not `FineTuneChatCompletionRequestAssistantMessage`"
            )
        else:
            match += 1
        ...
        if match > 1:
            # more than 1 match
            raise ValueError(
                "Multiple matches found when setting `actual_instance` in FineTuneChatRequestInputMessagesInner with oneOf schemas: ChatCompletionRequestFunctionMessage, ChatCompletionRequestSystemMessage, ChatCompletionRequestToolMessage, ChatCompletionRequestUserMessage, FineTuneChatCompletionRequestAssistantMessage. Details: "
                + ", ".join(error_messages)
            )
        elif match == 0:
            # no match
            raise ValueError(
                "No match found when setting `actual_instance` in FineTuneChatRequestInputMessagesInner with oneOf schemas: ChatCompletionRequestFunctionMessage, ChatCompletionRequestSystemMessage, ChatCompletionRequestToolMessage, ChatCompletionRequestUserMessage, FineTuneChatCompletionRequestAssistantMessage. Details: "
                + ", ".join(error_messages)
            )
        else:
            return v
