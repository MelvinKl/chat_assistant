...
from assistant.models.modify_thread_request_tool_resources import ModifyThreadRequestToolResources
try:
...
class ThreadObject(BaseModel):
    ...
    object: StrictStr = Field(description="The object type, which is always `thread`.")
    created_at: StrictInt = Field(description="The Unix timestamp (in seconds) for when the thread was created.")
    tool_resources: Optional[ModifyThreadRequestToolResources]
    ...
    __properties: ClassVar[List[str]] = ["id", "object", "created_at", "tool_resources", "metadata"]

    @field_validator('object')
    def object_validate_enum(cls, value):
        """Validates the enum"""
        if value not in ('thread',):
            raise ValueError("must be one of enum values ('thread')")
        return value
    ...
    def from_dict(cls, obj: Dict) -> Self:
        ...
        _obj = cls.model_validate({
            ...
            "tool_resources": ModifyThreadRequestToolResources.from_dict(obj.get("tool_resources")) if obj.get("tool_resources") is not None else None,
