...
class WebSearchTool(BaseModel):
    ...
    type: StrictStr = Field(
        description="The type of the web search tool. One of: - `web_search_preview` - `web_search_preview_2025_03_11` "
    )
        ...
        _obj = cls.model_validate(
            {
                "type": obj.get("type"),
                "user_location": (
                    WebSearchToolUserLocation.from_dict(obj.get("user_location"))
                    if obj.get("user_location") is not None
                    else None
                ),
                "search_context_size": (
                    obj.get("search_context_size")
                    if obj.get("search_context_size") is not None
                    else WebSearchContextSize.MEDIUM
                ),
            }
        )
        return _obj
