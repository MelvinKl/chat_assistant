...
class InviteRequest(BaseModel):
    ...
    projects: Optional[List[InviteRequestProjectsInner]] = Field(default=None, description="An array of projects to which membership is granted at the same time the org invite is accepted. If omitted, the user will be invited to the default project for compatibility with legacy behavior.")
    __properties: ClassVar[List[str]] = ["email", "role", "projects"]

    @field_validator('role')
    def role_validate_enum(cls, value):
        """Validates the enum"""
        if value not in ('reader', 'owner',):
    ...
    def from_dict(cls, obj: Dict) -> Self:
        ...
        _obj = cls.model_validate({
            "email": obj.get("email"),
            "role": obj.get("role"),
            "projects": [InviteRequestProjectsInner.from_dict(_item) for _item in obj.get("projects")] if obj.get("projects") is not None else None
        })
        return _obj
