...
class Response(BaseModel):
    ...
    created_at: Union[StrictFloat, StrictInt] = Field(
    ...
    output_text: Optional[StrictStr] = Field(
        default=None,
        description="SDK-only convenience property that contains the aggregated text output  from all `output_text` items in the `output` array, if any are present.  Supported in the Python and JavaScript SDKs. ",
    )
        ...
        _obj = cls.model_validate(
            {
                ...
                "incomplete_details": (
                    ResponseAllOfIncompleteDetails.from_dict(obj.get("incomplete_details"))
                    if obj.get("incomplete_details") is not None
                    else None
                ),
                "output": (
                    [OutputItem.from_dict(_item) for _item in obj.get("output")]
