...
class RealtimeServerEventConversationItemRetrieved(BaseModel):
    ...
    type: StrictStr = Field(description="The event type, must be `conversation.item.retrieved`.")
    item: RealtimeConversationItem
    __properties: ClassVar[List[str]] = ["event_id", "type", "item"]

    @field_validator("type")
    def type_validate_enum(cls, value):
        """Validates the enum"""
        if value not in ("conversation.item.retrieved",):
            raise ValueError("must be one of enum values ('conversation.item.retrieved')")
        return value
