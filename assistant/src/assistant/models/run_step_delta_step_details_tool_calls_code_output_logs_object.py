...
class RunStepDeltaStepDetailsToolCallsCodeOutputLogsObject(BaseModel):
    ...
    type: StrictStr = Field(description="Always `logs`.")
    logs: Optional[StrictStr] = Field(default=None, description="The text output from the Code Interpreter tool call.")
    __properties: ClassVar[List[str]] = ["index", "type", "logs"]

    @field_validator("type")
    def type_validate_enum(cls, value):
        """Validates the enum"""
        if value not in ("logs",):
            raise ValueError("must be one of enum values ('logs')")
        return value
    ...
    def from_dict(cls, obj: Dict) -> Self:
        ...
        _obj = cls.model_validate({"index": obj.get("index"), "type": obj.get("type"), "logs": obj.get("logs")})
        return _obj
