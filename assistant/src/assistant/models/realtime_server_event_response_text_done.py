...
class RealtimeServerEventResponseTextDone(BaseModel):
    ...
    type: StrictStr = Field(description="The event type, must be `response.text.done`.")
    ...
    text: StrictStr = Field(description="The final text content.")
    __properties: ClassVar[List[str]] = ["event_id", "type", "response_id", "item_id", "output_index", "content_index", "text"]

    @field_validator('type')
    def type_validate_enum(cls, value):
        """Validates the enum"""
        if value not in ('response.text.done',):
            raise ValueError("must be one of enum values ('response.text.done')")
        return value
