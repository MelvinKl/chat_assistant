...
class RealtimeServerEventResponseAudioDelta(BaseModel):
    ...
    type: StrictStr = Field(description="The event type, must be `response.audio.delta`.")
    ...
    delta: StrictStr = Field(description="Base64-encoded audio data delta.")
    __properties: ClassVar[List[str]] = ["event_id", "type", "response_id", "item_id", "output_index", "content_index", "delta"]

    @field_validator('type')
    def type_validate_enum(cls, value):
        """Validates the enum"""
        if value not in ('response.audio.delta',):
            raise ValueError("must be one of enum values ('response.audio.delta')")
        return value
