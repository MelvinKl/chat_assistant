...
class RealtimeClientEventConversationItemRetrieve(BaseModel):
    ...
    type: StrictStr = Field(description="The event type, must be `conversation.item.retrieve`.")
    item_id: StrictStr = Field(description="The ID of the item to retrieve.")
    __properties: ClassVar[List[str]] = ["event_id", "type", "item_id"]

    @field_validator("type")
    def type_validate_enum(cls, value):
        """Validates the enum"""
        if value not in ("conversation.item.retrieve",):
            raise ValueError("must be one of enum values ('conversation.item.retrieve')")
        return value
    ...
    def from_dict(cls, obj: Dict) -> Self:
        ...
        _obj = cls.model_validate(
            {"event_id": obj.get("event_id"), "type": obj.get("type"), "item_id": obj.get("item_id")}
        )
        return _obj
