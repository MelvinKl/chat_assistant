...
class CreateEmbeddingRequest(BaseModel):
    ...
    encoding_format: Optional[StrictStr] = Field(
        default="float",
        description="The format to return the embeddings in. Can be either `float` or [`base64`](https://pypi.org/project/pybase64/).",
    )
    dimensions: Optional[Annotated[int, Field(strict=True, ge=1)]] = Field(
        default=None,
    ...
    def from_dict(cls, obj: Dict) -> Self:
        ...
        _obj = cls.model_validate(
            {
                "input": (
                    CreateEmbeddingRequestInput.from_dict(obj.get("input")) if obj.get("input") is not None else None
                ),
                "model": (
                    CreateEmbeddingRequestModel.from_dict(obj.get("model")) if obj.get("model") is not None else None
                ),
