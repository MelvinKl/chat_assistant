...
class RealtimeServerEventResponseAudioDone(BaseModel):
    ...
    type: StrictStr = Field(description="The event type, must be `response.audio.done`.")
    ...
    __properties: ClassVar[List[str]] = ["event_id", "type", "response_id", "item_id", "output_index", "content_index"]

    @field_validator('type')
    def type_validate_enum(cls, value):
        """Validates the enum"""
        if value not in ('response.audio.done',):
            raise ValueError("must be one of enum values ('response.audio.done')")
        return value
