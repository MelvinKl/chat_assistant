...
from assistant.models.run_completion_usage import RunCompletionUsage
from assistant.models.run_object_incomplete_details import RunObjectIncompleteDetails
from assistant.models.run_object_last_error import RunObjectLastError
from assistant.models.run_object_required_action import RunObjectRequiredAction
...
class RunObject(BaseModel):
    ...
    tools: Annotated[List[AssistantObjectToolsInner], Field(max_length=20)] = Field(
    ...
    temperature: Optional[Union[StrictFloat, StrictInt]] = Field(
        default=None, description="The sampling temperature used for this run. If not set, defaults to 1."
    )
    top_p: Optional[Union[StrictFloat, StrictInt]] = Field(
        default=None, description="The nucleus sampling value used for this run. If not set, defaults to 1."
    )
    max_prompt_tokens: Optional[Annotated[int, Field(strict=True, ge=256)]] = Field(
        description="The maximum number of prompt tokens specified to have been used over the course of the run. "
    )
    max_completion_tokens: Optional[Annotated[int, Field(strict=True, ge=256)]] = Field(
    ...
    parallel_tool_calls: StrictBool = Field(
    ...
    def status_validate_enum(cls, value):
        ...
        if value not in (
            ...
            raise ValueError(
                "must be one of enum values ('queued', 'in_progress', 'requires_action', 'cancelling', 'cancelled', 'failed', 'completed', 'incomplete', 'expired')"
            )
        return value
        ...
        _obj = cls.model_validate(
            {
                ...
                "required_action": (
                    RunObjectRequiredAction.from_dict(obj.get("required_action"))
                    if obj.get("required_action") is not None
                    else None
                ),
                "last_error": (
                    RunObjectLastError.from_dict(obj.get("last_error")) if obj.get("last_error") is not None else None
                ),
                ...
                "incomplete_details": (
                    RunObjectIncompleteDetails.from_dict(obj.get("incomplete_details"))
                    if obj.get("incomplete_details") is not None
                    else None
                ),
                "model": obj.get("model"),
                "instructions": obj.get("instructions"),
                "tools": (
                    [AssistantObjectToolsInner.from_dict(_item) for _item in obj.get("tools")]
                ...
                "truncation_strategy": (
                    CreateRunRequestTruncationStrategy.from_dict(obj.get("truncation_strategy"))
                    if obj.get("truncation_strategy") is not None
                    else None
                ),
                "tool_choice": (
                    CreateRunRequestToolChoice.from_dict(obj.get("tool_choice"))
                ...
                "response_format": (
                    AssistantsApiResponseFormatOption.from_dict(obj.get("response_format"))
