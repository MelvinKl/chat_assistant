...
class RunObject(BaseModel):
    ...
    status: StrictStr = Field(description="The status of the run, which can be either `queued`, `in_progress`, `requires_action`, `cancelling`, `cancelled`, `failed`, `completed`, `incomplete`, or `expired`.")
    ...
    parallel_tool_calls: StrictBool = Field(description="Whether to enable [parallel function calling](/docs/guides/function-calling#configuring-parallel-function-calling) during tool use.")
    ...
    def status_validate_enum(cls, value):
        """Validates the enum"""
        if value not in ('queued', 'in_progress', 'requires_action', 'cancelling', 'cancelled', 'failed', 'completed', 'incomplete', 'expired',):
            raise ValueError("must be one of enum values ('queued', 'in_progress', 'requires_action', 'cancelling', 'cancelled', 'failed', 'completed', 'incomplete', 'expired')")
        return value
    ...
    def from_dict(cls, obj: Dict) -> Self:
        ...
        _obj = cls.model_validate({
            ...
            "truncation_strategy": CreateRunRequestTruncationStrategy.from_dict(obj.get("truncation_strategy")) if obj.get("truncation_strategy") is not None else None,
            "tool_choice": CreateRunRequestToolChoice.from_dict(obj.get("tool_choice")) if obj.get("tool_choice") is not None else None,
