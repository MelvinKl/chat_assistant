...
from assistant.models.file_search_tool_call_results_inner import FileSearchToolCallResultsInner
try:
...
class FileSearchToolCall(BaseModel):
    ...
    status: StrictStr = Field(description="The status of the file search tool call. One of `in_progress`,  `searching`, `incomplete` or `failed`, ")
    queries: List[StrictStr] = Field(description="The queries used to search for files. ")
    results: Optional[List[FileSearchToolCallResultsInner]] = Field(default=None, description="The results of the file search tool call. ")
    ...
    def status_validate_enum(cls, value):
        """Validates the enum"""
        if value not in ('in_progress', 'searching', 'completed', 'incomplete', 'failed',):
            raise ValueError("must be one of enum values ('in_progress', 'searching', 'completed', 'incomplete', 'failed')")
        return value
    ...
    def from_dict(cls, obj: Dict) -> Self:
        ...
        _obj = cls.model_validate({
            ...
            "results": [FileSearchToolCallResultsInner.from_dict(_item) for _item in obj.get("results")] if obj.get("results") is not None else None
        })
        return _obj
