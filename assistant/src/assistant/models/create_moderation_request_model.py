...
class CreateModerationRequestModel(BaseModel):
    ...
    any_of_schemas: List[str] = Literal[CREATEMODERATIONREQUESTMODEL_ANY_OF_SCHEMAS]
    ...
    def actual_instance_must_validate_anyof(cls, v):
        ...
        if error_messages:
            # no match
            raise ValueError(
                "No match found when setting the actual_instance in CreateModerationRequestModel with anyOf schemas: str. Details: "
                + ", ".join(error_messages)
            )
        else:
            return v
    ...
    def from_json(cls, json_str: str) -> Self:
        ...
        if error_messages:
            # no match
            raise ValueError(
                "No match found when deserializing the JSON string into CreateModerationRequestModel with anyOf schemas: str. Details: "
                + ", ".join(error_messages)
            )
        else:
            return instance
