...
class RealtimeServerEventResponseAudioTranscriptDone(BaseModel):
    ...
    type: StrictStr = Field(description="The event type, must be `response.audio_transcript.done`.")
    ...
    transcript: StrictStr = Field(description="The final transcript of the audio.")
    __properties: ClassVar[List[str]] = ["event_id", "type", "response_id", "item_id", "output_index", "content_index", "transcript"]

    @field_validator('type')
    def type_validate_enum(cls, value):
        """Validates the enum"""
        if value not in ('response.audio_transcript.done',):
            raise ValueError("must be one of enum values ('response.audio_transcript.done')")
        return value
